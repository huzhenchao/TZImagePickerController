...
-(UIImage *)getCoverImage:(NSURL *)outMovieURL atTime:(CGFloat)time isKeyImage:(BOOL)isKeyImage{
    ...
    self.assetImageGenerator = [[AVAssetImageGenerator alloc] initWithAsset:asset];
    _assetImageGenerator.appliesPreferredTrackTransform = YES;
    _assetImageGenerator.apertureMode = AVAssetImageGeneratorApertureModeEncodedPixels;
    ...
    if (!isKeyImage) {
        _assetImageGenerator.requestedTimeToleranceAfter = kCMTimeZero;
        _assetImageGenerator.requestedTimeToleranceBefore = kCMTimeZero;
        myTime = CMTimeMake(time*30,30);
    }
    
    thumbnailImageRef = [_assetImageGenerator copyCGImageAtTime:myTime actualTime:NULL error:nil];
...
-(void)getCoverImageArr:(NSURL *)outMovieURL imgSize:(CGSize)size callBackHandler:(void(^)(UIImage *img,NSInteger index))outHandler{
    
    CGFloat totalTime = [TJMediaManager getVideoTimeWithURL:outMovieURL];
    ...
    _assetImageGenerator.appliesPreferredTrackTransform = YES;
    _assetImageGenerator.apertureMode = AVAssetImageGeneratorApertureModeEncodedPixels;
    //tips:下面代码控制取图的时间点是否为关键帧，系统为了性能是默认取关键帧图片的
    _assetImageGenerator.requestedTimeToleranceAfter = kCMTimeZero;
    _assetImageGenerator.requestedTimeToleranceBefore = kCMTimeZero;
    //定义最大尺寸
    _assetImageGenerator.maximumSize = size;
    
    NSMutableArray *times = [NSMutableArray array];
    for (NSInteger i = 0; i<totalTime; i++) {
        CMTime  myTime = CMTimeMake(i*24, 24);
        [times addObject:[NSValue valueWithCMTime:myTime]];
    }
    [_assetImageGenerator generateCGImagesAsynchronouslyForTimes:times completionHandler:^(CMTime requestedTime, CGImageRef image, CMTime actualTime, AVAssetImageGeneratorResult result, NSError *error) {
        
        UIImage *thumbnailImage = [[UIImage alloc]init];
         if (result == AVAssetImageGeneratorSucceeded) {
             thumbnailImage = image ? [[UIImage alloc] initWithCGImage:image] : [[UIImage alloc]init];
         }
        if (outHandler) {
            outHandler(thumbnailImage,requestedTime.value/24);
        }
     }];
}
...
-(void)clearResource{
    ...
    [self.assetImageGenerator cancelAllCGImageGeneration];
